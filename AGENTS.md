...
- 状态：灼烧/中毒**可叠加层数**（重复附加层数相加），每回合结算 ceil(层数/2) 层（向上进位）并扣等量伤害、归 0 消失；战吼 伤害+2/2R、铁刺 伤害-1/2R、眩晕（跳过行动）。**技能封印（skillSeal）**：封印目标指定数量的随机技能，封印期间无法使用被封印的技能（`playerSkill` 拦截并消耗行动）；UI 技能按钮对被封印技能直接禁用（不可选中避免浪费行动点），战斗日志显示被封印的技能名，buff弹窗显示被封印的技能列表；敌方 AI（`selectEnemyAction`）同样将被封印技能排除出候选池，避免选到被封印技能导致行动被吞；若封印后剩余技能因不满足使用条件（如仅剩治疗且满血）导致候选池为空，AI 会从剩余技能中按 攻击>状态>增益>治疗 优先级兜底施放一个，避免「无技能可用」瘫痪。**战吼施放回合不计**：战吼（atkUp）`turns: N` 施放回合生效后还持续 N 个完整回合（实际跨越 N+1 回合）；铁刺/嘲讽/眩晕等负面状态施放回合正常计入。侵蚀节点：速度-2（入场）或 我方受伤+2（**多段每段都 +2**）或 每回合结束受到 2 点伤害。
...
- 开局 2 只宠物（御三家之一 + 随机同伴），队伍上限 8、出战 5；队伍已满时驯服/孵化/招募的新宠物进「处理队伍」界面（替换/融合/放生，队伍有空位时可直接加入；融合时待处理宠物也可作为同物种材料；熟练度远征模式融合选项不可用）。
...
- **熟练度远征模式**：独立 Roguelike 爬塔模式（`runMode: 'proficiency'`），从主菜单「⚔️ 熟练度远征」进入。核心机制：
  ...
  - **地图**：线性 50 层，每层 1 个节点（战斗/精英/事件/商店/铁匠铺/竞技场/奇遇/休憩），层15/30为随机Boss，层50为成长之主。成长系统属性递增消耗（HP: 2→10 点/次，SPD: 2→5 点/次）；强化技能 8/10/12 点（每技能最多3级，攻击+2伤害/治疗+2/buff+1效果值）。
  - **成长系统**：战斗获得成长点（每场+2参与+2存活），击杀奖励（普通+2/精英+4/Boss+6）。成长点分配属性（HP递增2→10点/次+2HP，SPD递增2→5点/次+1SPD）或解锁技能槽/替换技能/强化技能（8/10/12点，攻击+2伤害/连击+1段数/混合+1伤害+1效果/buff+1效果值，每技能最多3级，连击/速度类最多2级，传奇技能消耗×1.5，还原石重置返还50%）。**成长点转移**：将1只宠物的成长点转移给另1只，手续费=转移量的50%（向上取整），例转移10点手续费5点目标获5点。设计意图：给玩家"纠错"的机会，但手续费防止滥用。**商店商品**：成长之书（小）12金+1点、成长之书（中）22金+2点、成长之书（大）30金+3点、技能槽解锁50金、遗忘之石30金、治疗圣水30金、还原石25金、技能强化石40金、宠物招募20金、复活石60金（复活1只死亡宠物保留50%属性）。**背包道具专用屏幕**：技能强化石/还原石从背包使用时，进入专属屏幕（`EnhanceStoneScreen`/`EnhanceResetScreen`），直接选择宠物执行，无需走铁匠铺流程。
  - **竞技场三模式**：`Arena3Screen` 显示「模拟战（镜像）」「挑战赛」「表演赛」三选一。模拟战：镜像我方阵容，无伤亡无回血，失败无惩罚（自动返回地图+toast），胜利全员+5成长点；挑战赛：3精英，胜利+50金+全员+3成长点，**失败与其他战斗关一致——远征直接结束进入结算界面**（不承受随机坏事件代价，宠物正常阵亡）；表演赛：20金+指定宠物+5成长点，失败扣20金。
  - **敌人缩放**：普通敌人按层数区间缩放（1~15层 +8%/层，16~30层 +10%/层，31~50层 +15%/层），Boss不缩放。**敌人物种池按层数解锁**：1~10层普通池=一阶（迅迅/泡泡/灼灼/铁墩/咪咪/刺刺）+精英池=二阶；11~25层普通池=一阶60%+二阶40%+精英池=二阶60%+传奇40%；26~40层普通池=一阶20%+二阶80%+精英池=二阶30%+传奇70%；41~50层普通池=二阶50%+传奇50%+精英池=双传奇。一阶(rank1)：迅迅/泡泡/灼灼/铁墩/咪咪/刺刺；二阶(rank2)：迅牙/泡泡将/灼刃/棘尾/铁卫/蟒影；传奇(rank3)：迅天/泡泡龙神/灼天/铁壁神/深渊蛇王/棘刺王。
  - **回复**：战后回血80%，休息点100%+融合（继承副宠50%基础属性+30%成长点+可学技能），商店药水50%。
  - **专属数据文件**：`core/growth.ts`（等级/成长点）、`core/growth-map.ts`（地图生成）、`data/growth-events.ts`（10个专属事件）、`data/growth-shop.ts`（9种商店商品）、`data/growth-special.ts`（6种奇遇奖励）、`data/growth-boss.ts`（Boss数据）、`data/growth-arena.ts`（竞技场怪物池）。
  - **流程控制**：`freshGrowthRun()` 创建初始状态 → `enterNode()` 使用专属事件/商店/奇遇/Boss → 战斗后 `settleBattleGrowth()` 结算成长点 → Boss 击败或失败进入 `growth-result` 结算界面。
  - **UI 组件**：`GrowthScreen.tsx`（成长点分配）、`BlacksmithScreen.tsx`（铁匠铺·技能强化）、`Arena3Screen.tsx`（竞技场三模式）、`EnhanceStoneScreen.tsx`（技能强化石背包屏幕）、`EnhanceResetScreen.tsx`（还原石背包屏幕）、`GrowthResultScreen.tsx`（结算界面）。
  - **Boss 3（层50）成长之主**：80HP/8SPD，初始5成长值，被动「成长值」（主动攻击命中+2/回合结束+1，每5值受伤-1最多-5；**命中加成与技能成长值消耗互相独立**，若成长值不足则无法施放消耗技能）。技能：成长束缚（8伤害+封印2技能）、成长战吼（+3伤害2回合）、终焉成长（消耗10值回复20HP+永久+1伤害，每次使用累加，限3次最多+3）、成长冲击（消耗3值12伤害）、成长爆发（消耗3值全体8伤害）、成长召唤（消耗3值召唤傀儡）。小怪「成长傀儡」10HP/2SPD，被动「献祭」（回合结束+1值，死亡+3值），技能：傀儡束缚（5伤害+封印1技能）、傀儡拳（8伤害）、归魂（全体6伤害后自爆）。成长之主+1只傀儡时每回合结束获得2点成长值（自身+1+傀儡献祭+1）。
