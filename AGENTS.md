...
- **熟练度远征模式**：独立 Roguelike 爬塔模式（`runMode: 'proficiency'`），从主菜单「⚔️ 熟练度远征」进入。核心机制：
  ...
  - **成长系统**：战斗获得成长点（每场+2参与+2存活），击杀奖励（普通+2/精英+4/Boss+6）。成长点分配属性（+2HP/+1SPD各2点）或解锁技能槽/替换技能。
  - **敌人缩放**：每层+10%生命、+5%速度；层1-5固定1只，层6-10最多2只，层11-15最多3只；精英全程2只（仅品阶2）；Boss 40HP + 小怪。
