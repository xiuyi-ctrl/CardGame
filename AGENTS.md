...
- **熟练度远征模式**：独立 Roguelike 爬塔模式（`runMode: 'proficiency'`），从主菜单「⚔️ 熟练度远征」进入。核心机制：
  ...
  - **Boss 3（层50）成长之主**：80HP/8SPD，初始5成长值，被动「成长值」（攻击命中+2/回合结束+1，每5值受伤-1最多-5）。技能：成长束缚（8伤害+封印2技能）、成长战吼（+3伤害2回合）、终焉成长（消耗10值回复20HP+永久+1伤害，限3次）、成长冲击（消耗3值12伤害）、成长爆发（消耗3值全体8伤害）、成长召唤（消耗3值召唤傀儡）。小怪「成长傀儡」10HP/2SPD，被动「献祭」（回合结束+1值，死亡+3值），技能：傀儡束缚（5伤害+封印1技能）、傀儡拳（8伤害）、归魂（全体6伤害后自爆）。成长之主+1只傀儡时每回合结束获得2点成长值（自身+1+傀儡献祭+1）。
