...
- **幕间统计与评级系统**：`RunStats`（`game.ts`）跟踪全局累计数据：`battlesWon/goldEarned/petsTamed/petsLost/turnsPlayed` + `lastBattleRound`（按回合计数）+ `actSnapshot`（幕开始时快照）。每次战斗胜利后 `resolveBattle` 更新 `battlesWon/goldEarned/petsTamed/petsLost`；`PLAYER_SKILL` 按 `battle.round` 变化增量更新 `turnsPlayed`；事件战斗胜利同样更新。击败首领后进入 `inter_act` 幕间统计界面，显示本幕增量（当前累计 - 快照）。`INTER_ACT_CONTINUE` 快照当前累计值并重置 `lastBattleRound`。通关/失败时 `computeRating` 按 4 维度评分（效率 30 / 生存 30 / 参与 20 / 资源 20），输出 S/A/B/C/D 评级。`createInitialState` 包含默认 `runStats`（防止 `DEBUG_JUMP` 等路径丢失）；`LOAD_GAME` 对旧存档补全缺失 `runStats`；`isValidGameState` 包含 `inter_act` 屏幕。
