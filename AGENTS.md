...
- **事件系统升级**：事件模板从 6 个扩展到 15 个，新增幕次分层机制（幕1 池 8 个、幕2-3 池 12 个）。新增 6 种 EventChoice 类型：`battle`（触发战斗）、`sacrifice`（献祭宠物）、`boost`（永久属性提升）、`purify`（清除诅咒）、`curse`（附加诅咒）、`status`（附加状态）。赌博类事件结果在生成时用种子预掷，可复现。战斗交互事件使用当前幕怪物池生成敌人，胜利获奖励，失败全体-15%血。
